 ...
 *              найти следующее синхронизирующее слово с байтами в необработанном mp3-потоке
 * 
 * Inputs:      buffer to search for sync word
 *              max number of bytes to search in buffer
 * Входы: буфер для поиска синхронизирующего слова
 * максимальное количество байтов для поиска в буфере
 *
 * Outputs:     none
 *
 * Return:      offset to first sync word (bytes from start of buf)
 *              -1 if sync not found after searching nBytes
 * Возврат: смещение к первому слову синхронизации (байты от начала буфера)
 * -1, если синхронизация не найдена после поиска в nBytes
 **************************************************************************************/
 ...
 *              -Количество действительных байтов, оставшихся в inbuf
 *              -указатель на OutBuf, достаточно большой, чтобы провести один кадр
                      из декодированных семплов PCM
