...
// Funktion : FATFS-Dateisystem fьr SD-Medien
//            LoLevel-IO-Modul
//            Quelle = STM-Files
...
static void NVIC_Configuration(void)
{
  ...
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = (uint8_t)SD_SDIO_IRQn_PRIO;
  NVIC_InitStructure.NVIC_IRQChannelSubPriority = (uint8_t)SD_SDIO_IRQn_PRIO_SUB;
  NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
  NVIC_Init(&NVIC_InitStructure);
#if defined (SD_DMA_MODE)
  NVIC_InitStructure.NVIC_IRQChannel = SD_SDIO_DMA_IRQn;
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = (uint8_t)SD_SDIO_DMA_IRQn_PRIO;
  NVIC_InitStructure.NVIC_IRQChannelSubPriority = (uint8_t)SD_SDIO_DMA_IRQn_PRIO_SUB;
  NVIC_Init(&NVIC_InitStructure);
#endif
}
//--------------------------------------------------------------
...
uint8_t SD_Detect(void)
{
  ...
  uint8_t sta=GPIO_ReadInputDataBit(SD_DETECT_GPIO_PORT, SD_DETECT_PIN);
  if (sta != (uint8_t)Bit_RESET)
  {
...
SD_Error SD_ReadMultiBlocksFIXED(uint8_t *readbuff, uint32_t ReadAddr, uint32_t BlockSize, uint32_t NumberOfBlocks)
{
  ...
  int i=10000000;
  while (SD_SDIO_DMA_STREAM->CR & DMA_SxCR_EN && i--) {};
...
static SD_Error CmdResp1Error(uint8_t cmd)
{
  ...
  while (!(status & (SDIO_FLAG_CCRCFAIL | SDIO_FLAG_CMDREND | SDIO_FLAG_CTIMEOUT)) && timeout--)
  {
    status = SDIO->STA;
  }

  if (status & SDIO_FLAG_CTIMEOUT || timeout==0)
  {
    errorstatus = SD_CMD_RSP_TIMEOUT;
    SDIO_ClearFlag(SDIO_FLAG_CTIMEOUT);
    return(errorstatus);
...
void SD_LowLevel_Init(void)
{
  ...
  /* Configure PC.08, PC.09, PC.10, PC.11 pins: D0, D1, D2, D3 pins */
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8 | GPIO_Pin_9 | GPIO_Pin_10 | GPIO_Pin_11;
  GPIO_InitStructure.GPIO_Speed = SDIO_GPIO_SPEED;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
  GPIO_InitStructure.GPIO_PuPd = SDIO_PINS_PuPd_TYPE;
  GPIO_Init(GPIOC, &GPIO_InitStructure);
#else
  GPIO_PinAFConfig(GPIOC, GPIO_PinSource8, GPIO_AF_SDIO);
  GPIO_PinAFConfig(GPIOC, GPIO_PinSource12, GPIO_AF_SDIO);
...
#if USE_DETECT_IRQ==1
  //detect EXTI
  SYSCFG_EXTILineConfig(SD_DETECT_EXTI_PORT,SD_DETECT_EXTI_PINSRC);
  ...
  exti_detect.EXTI_Trigger = SD_DETECT_EXTI_TRIGGER;  // срабатываем по спаду импульса
  exti_detect.EXTI_LineCmd = ENABLE;    // вкл
  EXTI_Init(&exti_detect);
  
  NVIC_InitTypeDef nvic_detect;
  nvic_detect.NVIC_IRQChannel = SD_DETECT_EXTI_IRQChannel;
  nvic_detect.NVIC_IRQChannelPreemptionPriority = SD_DETECT_EXTI_Priority;  
  nvic_detect.NVIC_IRQChannelSubPriority = SD_DETECT_EXTI_SubPriority; 
  nvic_detect.NVIC_IRQChannelCmd = ENABLE;
  NVIC_Init(&nvic_detect);
#endif 
...
void SD_LowLevel_DMA_TxConfig(uint32_t *BufferSRC, uint32_t BufferSize)
{
  ...
  while (SD_SDIO_DMA_STREAM->CR & DMA_SxCR_EN) {};
  ...
  SDDMA_InitStructure.DMA_BufferSize = 0;//(uint32_t)(BufferSize/4);
  ...
  DMA_ITConfig(SD_SDIO_DMA_STREAM, DMA_IT_TC/*|DMA_IT_HT|DMA_IT_DME*/|DMA_IT_TE|DMA_IT_FE, ENABLE);
...
void SD_LowLevel_DMA_RxConfig(uint32_t *BufferDST, uint32_t BufferSize)
{
  ...
  DMA_StructInit(&SDDMA_InitStructure);
  //------------------------------------------------------
  //----------Ошибка в размере буфера-исправить!-----------
  SDDMA_InitStructure.DMA_Channel = SD_SDIO_DMA_CHANNEL;
  SDDMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)SDIO_FIFO_ADDRESS;
  ...
  SDDMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;
  SDDMA_InitStructure.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Word;
  SDDMA_InitStructure.DMA_MemoryDataSize = DMA_MemoryDataSize_Byte;//DMA_MemoryDataSize_Word;-ОШИБКА ЧТЕНИЯ ФЛЕШ
  SDDMA_InitStructure.DMA_Mode = DMA_Mode_Normal;
  SDDMA_InitStructure.DMA_Priority = DMA_Priority_VeryHigh;
  SDDMA_InitStructure.DMA_FIFOMode = DMA_FIFOMode_Enable;
  SDDMA_InitStructure.DMA_FIFOThreshold = DMA_FIFOThreshold_Full;
...
{
  if(SD_SDIO_DMAxISR & SD_SDIO_DMA_FLAG_TCIF)
  {
    DMA_ITConfig(SD_SDIO_DMA_STREAM, DMA_IT_TC/*|DMA_IT_HT|DMA_IT_DME*/|DMA_IT_TE|DMA_IT_FE, DISABLE);
  ...
  //uint32_t Number_of_data_transferred = ((uint32_t)0xFFFF)- SD_SDIO_DMA_STREAM->NDTR;
  //uint32_t Num_byte = Number_of_data_transferred * 4;
  //if(dma_buff_cnt==Num_byte)
