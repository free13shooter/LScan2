...
/*----MainWizard-dem1305-modification----*/
// VENDOR REQUESTS
/**
* @brief  USBD_StdVndReq
*         Handle vendor usb interface requests
* @param  pdev: USB OTG device instance
* @param  req: usb request
* @retval status
*/
USBD_Status  USBD_StdVndReq (USB_OTG_CORE_HANDLE  *pdev, USB_SETUP_REQ  *req)
{
  USBD_Status ret = USBD_OK; 
  ...
  case USB_OTG_CONFIGURED:
     ...
     if (req->bmRequest & 0x80)//направление - от устройства к хосту(IN DIRECTION)
     {
      if(ret == USBD_OK)
      {
         USB_OTG_EP0_OutStart(pdev); //перенастройка точки на прием
      }
      else//FAIL
      {
        USBD_CtlSendStatus(pdev);//USBD_CtlError(pdev , req);//чтоб устройсво не остановило точку
      }
     }
